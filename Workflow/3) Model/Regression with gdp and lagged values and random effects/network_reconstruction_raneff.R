...
for (tau in 1:length(t)){
  ...
  ll<-function(theta){
    ...
    theta_tilde<-theta[(n_sender+n_receiver+1):(n_sender+n_receiver+3)]
  ...
  sol_2<-c(0,0,0)
  nl<-nloptr(x0=rep(1,n_sender+n_receiver+3), eval_jac_g_eq =  cgradient, eval_f = ll,eval_grad_f = llgradient,eval_g_eq =constraint,opts = opts)
  sol_1<-nl$solution
  ...
  l_R<-function(sigma){
    ...
    if (det(V)==0){V<-as.matrix(nearPD(V)$mat)}
  ...
  while(crit>0){
    ...
    sol_1<-nl$solution
    sol_2<-optim(par=c(0,0,0),fn=l_R,method="Nelder-Mead")$par
    vec_E<-exp(Z%*%sol_1)
  ...
  # save results----
  save.image(paste("3) Model/Regression with gdp and lagged values and random effects/results/res",tau,".RData",sep=""))
